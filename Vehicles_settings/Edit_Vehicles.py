...
class Edit_Vehicles (BoxLayout):
    ...
    def check (self):
        if (self.select_id is None):
        ...
        elif len([x for x in self.text1.text if ((ord(x) >= ord('a') and ord(x) <= ord('z')) or (ord(x) >= ord('A') and ord(x) <= ord('Z')) or (ord(x) >= ord('0') and ord(x) <= ord('9')))]) != len(self.text1.text) or self.text1.text == "SPZ":
            ...
            self.call_Back()
class Editing (App):
    def build(self):return Edit_Vehicles()
