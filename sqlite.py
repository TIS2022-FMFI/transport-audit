...
def zmaz_zaznam(tabulka, id, ine=MAZACIA_KONSTANTA):
    ...
    if tabulka == "User":
        db.execute(
            f"UPDATE {tabulka} set doplnok = '{ine}', last_sync = '{str(datetime.datetime.now())}' WHERE code = {id}")
    else:
        db.execute(
            f"UPDATE {tabulka} set doplnok = '{ine}', last_sync = '{str(datetime.datetime.now())}' WHERE id = '{id}'")
    data['doplnok'] = ine
...
def vloz_vykonavatela(tabulka, id, ine=None):
    ...
    if tabulka == "User":
        db.execute(
            f"UPDATE {tabulka} set vykonavatel = {ine}, last_sync = '{str(datetime.datetime.now())}' WHERE code = {id}")
    else:
        db.execute(
            f"UPDATE {tabulka} set vykonavatel = {ine}, last_sync = '{str(datetime.datetime.now())}' WHERE id = '{id}'")
...
def synchronize_db_client_server():
    ...
    for nazov_tabulky in tabulky:
        ...
        vysledky_rekvestov.append(
            r)  # Aby som nesiel príliš rýchlo, potom sa nestihnú tabulky spracovat v spravnom poradí a to vedie k zlým foreign keys
...
class Customer:
    ...
    def update(self):
        if self.id:
            sql = """UPDATE CUSTOMER SET Name= '%s', last_sync = '%s' WHERE id = '%s'""" % (
            self.Name, parse(str(datetime.datetime.now())), self.id)
            cursor.execute(sql)
            cursor.execute("COMMIT")
            data = dict()
            data['id'] = self.id
...
class General():
    ...
    def nahraj(self, Last_changes, Last_available,
               Automatic_export):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        ...
        entry = cursor.fetchone()
        if entry is None:
            cursor.execute('INSERT INTO General (id, Last_changes, Last_available, Automatic_export) VALUES (?,?,?,?)',
                           (self.id, self.Last_changes, self.Last_available, self.Automatic_export))  #
            cursor.execute("COMMIT")
            data = dict()
    ...
    def update(self):
        if self.id:
            ...
            cursor.execute(sql, (
            self.Last_changes, self.Last_available, self.Automatic_export, parse(str(datetime.datetime.now())),
            self.id))
            cursor.execute("COMMIT")
            data = dict()
            data['Last_changes'] = str(self.Last_changes)
...
class Config():
    ...
    def nahraj(self, Customer_id, Vehicle_id):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        ...
        entry = cursor.fetchone()
        if entry is None:
            cursor.execute('INSERT INTO Config (id, Customer_id, Vehicle_id) VALUES (?,?,?)',
                           (self.id, self.Customer_id, self.Vehicle_id))  #
            cursor.execute("COMMIT")
            data = dict()
...
class User():
    ...
    def nahraj(self, Name, Last_name, User_Role_id):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        ...
        entry = cursor.fetchone()
        if entry is None:
            cursor.execute('INSERT INTO User (code, Name, Last_name, User_Role_id) VALUES (?,?,?,?)',
                           (self.code, self.Name, self.Last_name, self.User_Role_id))  #
            cursor.execute("COMMIT")
            data = dict()
            data['Name'] = self.Name
    ...
    def update(self):
        if self.code:
            ...
            cursor.execute(sql, (
            self.Name, self.Last_name, self.User_Role_id, parse(str(datetime.datetime.now())), self.code))
            cursor.execute("COMMIT")
            data = dict()
            data['Name'] = self.Name
            data['Last_name'] = self.Last_name
...
class Advanced_user():
    ...
    def nahraj(self, Config_id, User_code):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        ...
        entry = cursor.fetchone()
        if entry is None:
            cursor.execute('INSERT INTO Advanced_user (id, Config_id, User_code) VALUES (?,?,?)',
                           (self.id, self.Config_id, self.User_code))  #
            cursor.execute("COMMIT")
            data = dict()
...
class Pattern_Item():
    ...
    def nahraj(self, Number, Pattern_id,
               Stillage_type_id):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        self.Number = Number
    ...
    def update(self):
        if self.id:
            ...
            cursor.execute(sql, (
            self.Number, self.Pattern_id, self.Stillage_type_id, parse(str(datetime.datetime.now())), self.id))
            cursor.execute("COMMIT")
            data = dict()
            data['Number'] = self.Number
            data['id'] = self.id
...
class Shipment():
    ...
    def nahraj(self, User_code, Date_time_close, Customer_id,
               Vehicle_id):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        self.User_code = User_code
        ...
        if entry is None:
            cursor.execute(
                'INSERT INTO Shipment (id, Date_time_close, User_code, Customer_id, Vehicle_id) VALUES (?,?,?,?,?)',
    ...
    def update(self):
        if self.id:
            ...
            cursor.execute(sql, (self.User_code, self.Date_time_close, self.Customer_id, self.Vehicle_id,
                                 parse(str(datetime.datetime.now())), self.id))
            cursor.execute("COMMIT")
            data = dict()
            data['User_code'] = self.User_code
            data['Date_time_close'] = self.Date_time_close
...
class Work_statement():
    ...
    def nahraj(self, User_code, Work, Time_start,
               Time_end):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        self.Work = Work
        self.User_code = User_code
    ...
    def update(self):
        if self.id:
            ...
            cursor.execute(sql, (
            self.User_code, self.Work, self.Time_start, self.Time_end, parse(str(datetime.datetime.now())), self.id))
            cursor.execute("COMMIT")
            data = dict()
            data['User_code'] = self.User_code
            data['id'] = self.id
...
class Stillage():
    ...
    def nahraj(self, Date_time_start, Date_time_end, Stillage_number, Stillage_Number_on_Header, First_scan_product,
               Last_scan_product, JLR_Header_NO, Carriage_L_JLR_H, _Check, First_scan_TLS_code, Last_scan_TLS_code,
               Stillage_Type_id, Shipment_id, TLS_range_start, TLS_range_stop,
               Note):  # Ak záznam existuje vráti None, inak ho nahra do db a vrati class
        ...
        if entry is None:
            cursor.execute(
                'INSERT INTO Stillage (id, Date_time_start, Date_time_end, Stillage_number, Stillage_Number_on_Header, First_scan_product, Last_scan_product, JLR_Header_NO, Carriage_L_JLR_H, _Check, First_scan_TLS_code, Last_scan_TLS_code, Stillage_Type_id, Shipment_id, TLS_range_start, TLS_range_stop, Note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                (
                self.id, self.Date_time_start, self.Date_time_end, self.Stillage_number, self.Stillage_Number_on_Header,
                self.First_scan_product, self.Last_scan_product, self.JLR_Header_NO, self.Carriage_L_JLR_H, self._Check,
                self.First_scan_TLS_code, self.Last_scan_TLS_code, self.Stillage_Type_id, self.Shipment_id,
                self.TLS_range_start, self.TLS_range_stop, self.Note))  #
            cursor.execute("COMMIT")
            data = dict()
    ...
    def update(self):
        if self.id:
            ...
            cursor.execute(sql, (
            self.Date_time_start, self.Date_time_end, self.Stillage_number, self.Stillage_Number_on_Header,
            self.First_scan_product, self.Last_scan_product, self.JLR_Header_NO, self.Carriage_L_JLR_H, self._Check,
            self.First_scan_TLS_code, self.Last_scan_TLS_code, self.Stillage_Type_id, self.Shipment_id,
            self.TLS_range_start, self.TLS_range_stop, self.Note, parse(str(datetime.datetime.now())), self.id))
            cursor.execute("COMMIT")
            data = dict()
            data['Date_time_start'] = self.Date_time_start
            data['Date_time_end'] = self.Date_time_end
